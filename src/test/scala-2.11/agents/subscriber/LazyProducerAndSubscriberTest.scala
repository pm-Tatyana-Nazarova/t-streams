...
class LazyProducerAndSubscriberTest extends FlatSpec with Matchers with BeforeAndAfterAll with TestUtils{
  ...
  "Some amount of producers and subscriber" should "producers - send transactions in many partition" +
    ...
    val subscriber = new BasicSubscribingConsumer("test_consumer", streamInst, consumerOptions,SubscriberCoordinationOptions("localhost:8101",
      "/unit",
      List(new InetSocketAddress("localhost", 2181)), 7000),
      callback,
      path)
