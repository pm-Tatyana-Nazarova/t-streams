...
class LazyBasicProducerAndConsumerWithRedisLockerWithCassandraTest extends FlatSpec with Matchers with BeforeAndAfterAll{
  ...
  "two producers and consumer" should "first producer create transaction and start filling it lazily," +
    ...
    val lockService1 = new RedisLockerFactory("/some_path", config)
    val lockService2 = new RedisLockerFactory("/some_path", config)
    val lockService3 = new RedisLockerFactory("/some_path", config)
