package agents.both.batch_insert.cassandra_redis
...
class CR_BasicProducerAndConsumerLazyTest extends FlatSpec with Matchers with BeforeAndAfterAll with BatchSizeTestVal{
  ...
  val lockerFactoryForProducer1 = new RedisLockServiceFactory("/some_path", config)
  val lockerFactoryForProducer2 = new RedisLockServiceFactory("/some_path", config)
